...
public class FirstPersonController : MonoBehaviour
{
    ...
    #region Super Jump Variables

    [Header("Super Jump Settings")]
    public bool enableSuperJump = true; // Enable or disable the super jump feature
...
public TMPro.TextMeshProUGUI superJumpCooldownText; // NEW
    ...
    private void Start()
    {
        ...
        // Initialize the super jump cooldown text
        if (superJumpCooldownText != null)
        {
            superJumpCooldownText.text = "E for Super Jump";
        }
    ...
    private void Update()
    {
        ...
        if (enableSuperJump && Input.GetKeyDown(superJumpKey) && canUseSuperJump)
        {
            PerformSuperJump();
        }
    ...
    private IEnumerator SuperJumpCooldownRoutine()
    {
        canUseSuperJump = false;

        float cooldownRemaining = superJumpCooldown;
        while (cooldownRemaining > 0)
        {
            if (superJumpCooldownText != null)
            {
                superJumpCooldownText.text = Mathf.Ceil(cooldownRemaining).ToString() + "s";
            }
        ...
        if (superJumpCooldownText != null)
        {
            superJumpCooldownText.text = "Super Jump Ready!";
        }
...
EditorGUILayout.Space();
EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
GUILayout.Label("Super Jump", new GUIStyle(GUI.skin.label)
{
...
fpc.enableSuperJump = EditorGUILayout.ToggleLeft(new GUIContent("Enable Super Jump", "Determines if the player can use a super jump ability."), fpc.enableSuperJump);

GUI.enabled = fpc.enableSuperJump;
fpc.superJumpKey = (KeyCode)EditorGUILayout.EnumPopup(new GUIContent("Super Jump Key", "Key used to trigger the super jump."), fpc.superJumpKey);
fpc.superJumpForce = EditorGUILayout.Slider(new GUIContent("Super Jump Force", "How strong the upward force of the super jump is."), fpc.superJumpForce, 1f, 100f);
fpc.superJumpCooldown = EditorGUILayout.Slider(new GUIContent("Super Jump Cooldown", "How long to wait before using super jump again."), fpc.superJumpCooldown, 1f, 60f);
